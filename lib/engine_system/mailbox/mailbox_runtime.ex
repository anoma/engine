  ...
  def handle_cast({:enqueue_message, message}, state) do
    ...
    if is_internal_message do
      ...
      case execute_behaviour(dsl_message, state) do
        ...
        {:error, reason} ->
          IO.puts(
            "🔧 MailboxRuntime #{inspect(message.target)}: Internal message behavior execution failed: #{format_behaviour_error(reason)}"
          )
      ...
      case execute_behaviour(dsl_message, state) do
        ...
        {:error, reason} ->
          IO.puts(
            "🔧 MailboxRuntime #{inspect(message.target)}: Behavior execution failed: #{format_behaviour_error(reason)}"
          )
          {:noreply, [], state}
      end
    end
