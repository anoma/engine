  ...
  behaviour do
    ...
    on_message :forward_message, %{target: target, payload: payload}, config, env, sender do
      if env.counter < config.max_forwards do
        ...
        IO.puts("📨 DiagramDemo: Forwarding #{inspect(payload)} to #{inspect(target)} (#{new_env.counter}/#{config.max_forwards})")
    ...
    on_message :broadcast, %{message: message}, config, env, sender do
      if config.broadcast_enabled and length(env.targets) > 0 do
        ...
        send_effects = env.targets
        |> Enum.map(fn target ->
          {:send, target, message}
        end)
        
        IO.puts("📡 DiagramDemo: Broadcasting #{inspect(message)} to #{length(env.targets)} targets")
