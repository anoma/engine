defmodule Examples.DSLMailboxSimple do
    ...
    behaviour do
      on_message :put, %{key: key, value: value}, config, env, sender do
        if map_size(env.store) >= config.max_keys and not Map.has_key?(env.store, key) do
          ...
          new_access_counts = if config.enable_stats do
      ...
      on_message :get, %{key: key}, config, env, sender do
        ...
        new_env = if config.enable_stats do
    ...
    behaviour do
      on_message :enqueue_message, %{message: message}, _config, env, _sender do
        ...
        new_env = %{env |
          ...
          total_received: env.total_received + 1
        }
  ...
  defengine PriorityMailbox do
    ...
    behaviour do
      on_message :enqueue_message, %{message: message}, _config, env, _sender do
        ...
        case category do
          :write ->
            ...
            new_env = %{env |
              coalesce_buffer: new_coalesce_buffer,
              stats: new_stats
            }
            {:ok, [{:update_environment, new_env}]}
      ...
      on_message :flush_coalesced_writes, %{}, _config, env, _sender do
        ...
        new_buffer = Enum.reduce(env.coalesce_buffer, env.priority_buffer, fn {_key, message}, buffer ->
          timestamp = :erlang.system_time(:microsecond)
          priority_key = {2, timestamp, env.stats.received}  # Write priority = 2
  ...
  def unified_example do
    ...
    {:ok, addr1} = EngineSystem.spawn_engine(
      KVProcessingEngine,              # Processing engine (DSL-defined)
      %{access_mode: :read_write},     # Processing config
      %{store: %{}},                   # Processing environment
      :kv_with_fifo,                   # Name
      SimpleFIFOMailbox,               # Mailbox engine (ALSO DSL-defined!)
      %{batch_size: 10}                # Mailbox config
    )

    # Spawn the same processing engine with priority mailbox
    {:ok, addr2} = EngineSystem.spawn_engine(
      KVProcessingEngine,              # Same processing engine
      %{access_mode: :read_write},
      %{store: %{}},
      :kv_with_priority,
      PriorityMailbox,                 # Different mailbox engine (ALSO DSL-defined!)
  ...
  def standalone_mailbox_example do
    ...
    {:ok, mailbox_addr} = EngineSystem.spawn_engine(
      SimpleFIFOMailbox,               # This is a mailbox engine
      %{batch_size: 20, max_demand: 200}, # Configuration
      %{},                             # Environment
      :standalone_queue                # Name
    )
  ...
  def compare_mailbox_stats do
    ...
    %{
      ...
      architecture: "Both engines use the same processing logic with different mailbox delivery policies!"
    }
  end
end
